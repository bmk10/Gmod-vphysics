...
class CDefaultCarWheelTracer : public IPhysicsVehicleWheelTrace {
	public:
		CDefaultCarWheelTracer(CPhysicsSCarVehicleController *pVehicle, IPhysicsGameTrace *pGameTrace):
		m_pVehicle(pVehicle),
	...
	private:
		CPhysicsSCarVehicleController *m_pVehicle;
...
class CSCarRaycaster : public btVehicleRaycaster {
	public:
		CCarRaycaster(btDynamicsWorld *pWorld, CPhysicsSCarVehicleController *pController) {
			m_pWorld = pWorld;
	...
	private:
		...
		CPhysicsSCarVehicleController *	m_pController;
};
...
/*********************************
* CLASS CPhysicsSCarVehicleController
*********************************/

CPhysicsSCarVehicleController::CPhysicsSCarVehicleController(CPhysicsEnvironment *pEnv, CPhysicsObject *pBody, const vehicleparams_t &params, unsigned int nVehicleType, IPhysicsGameTrace *pGameTrace) {
	m_pEnv				= pEnv;
	m_pBody				= pBody;
...
CPhysicsSCarVehicleController::~CPhysicsSCarVehicleController() {
	ShutdownBullVehicle();
}

void CPhysicsSCarVehicleController::InitVehicleParams(const vehicleparams_t &params) {
	m_vehicleParams = params;
}

void CPhysicsSCarVehicleController::InitBullVehicle() {
...
void CPhysicsSCarVehicleController::ShutdownBullVehicle() {
...
void CPhysicsSCarVehicleController::InitCarWheels() {
	int wheelIndex = 0;
...
void CPhysicsSCarVehicleController::DestroyCarWheels() {
	for (int i = 0; i < m_iWheelCount; i++) {
		m_pEnv->DestroyObject(m_pWheels[i]);
		m_pWheels[i] = NULL;
	}
...
CPhysicsObject *CPhysicsSCarVehicleController::CreateWheel(int wheelIndex, vehicle_axleparams_t &axle) {
...
void CPhysicsSCarVehicleController::SetWheelForce(int wheelIndex, float force) {
...
void CPhysicsSCarVehicleController::SetWheelBrake(int wheelIndex, float brakeVal) {
	if (wheelIndex >= m_iWheelCount || wheelIndex < 0) {
		Assert(0);
		return;
	}
...
void CPhysicsSCarVehicleController::SetWheelSteering(int wheelIndex, float steerVal) {
	if (wheelIndex >= m_iWheelCount || wheelIndex < 0) {
		Assert(0);
		return;
	}
...
void CPhysicsSCarVehicleController::Update(float dt, vehicle_controlparams_t &controls) {
...
#ifndef USE_WHEELED_VEHICLE
void CPhysicsSCarVehicleController::UpdateSteering(vehicle_controlparams_t &controls, float dt) {
...
void CPhysicsSCarVehicleController::UpdateEngine(vehicle_controlparams_t &controls, float dt) {
...
void CPhysicsSCarVehicleController::UpdateWheels(vehicle_controlparams_t &controls, float dt) {
...
float CPhysicsSCarVehicleController::UpdateBooster(float dt) {
	NOT_IMPLEMENTED
	return 0.0f; // Return boost delay.
}

#ifndef USE_WHEELED_VEHICLE
void CPhysicsSCarVehicleController::CalcEngineTransmission(vehicle_controlparams_t &controls, float dt) {
...
void CPhysicsSCarVehicleController::CalcEngine(vehicle_controlparams_t &controls, float dt) {
	// Speed governor
...
CPhysicsObject *CPhysicsSCarVehicleController::GetBody() {
	return m_pBody;
}

int CPhysicsSCarVehicleController::GetWheelCount() {
	return m_iWheelCount;
}

IPhysicsObject *CPhysicsSCarVehicleController::GetWheel(int index) {
	if (index >= m_iWheelCount || index < 0) return NULL;

	return m_pWheels[index];
}

bool CPhysicsSCarVehicleController::GetWheelContactPoint(int index, Vector *pContactPoint, int *pSurfaceProps) {
...
void CPhysicsSCarVehicleController::SetSpringLength(int wheelIndex, float length) {
...
void CPhysicsSCarVehicleController::SetWheelFriction(int wheelIndex, float friction) {
	Assert(wheelIndex >= m_iWheelCount || wheelIndex < 0);
	if (wheelIndex >= m_iWheelCount || wheelIndex < 0) {
		return;
	}
...
void CPhysicsSCarVehicleController::SetPosition(const Vector *pos, const QAngle *ang) {
	if (!pos && !ang) return;
...
void CPhysicsSCarVehicleController::GetCarSystemDebugData(vehicle_debugcarsystem_t &debugCarSystem) {
...
void CPhysicsSCarVehicleController::VehicleDataReload() {
...
IPhysicsVehicleController *CreateVehicleController(CPhysicsEnvironment *pEnv, CPhysicsObject *pBody, const vehicleparams_t &params, unsigned int nVehicleType, IPhysicsGameTrace *pGameTrace) {
	...
	CPhysicsSCarVehicleController *pController = new CPhysicsSCarVehicleController(pEnv, pBody, params, nVehicleType, pGameTrace);
	return pController;
}